...
# # 两个 ndarray 对象维度中从低维度开始对比，如果有其中有一个 ndarray 对象该维度数组长度为1 或者两个 ndarray 对象该维度数组长度相等，则该维度满足算术运算规则，继续比较更高的维度，直到其中维度数低的 ndarray 对象的全部维度检验完毕（当然两个 ndarray 对象维度相等需要全部维度检验完毕），必须全部维度都满足运算规则，否则两个 ndarray 对象不能进行算术运算

...
meter     标量测量标签，即根据具体的属性值最终分为警告、异常、正常段落的测量数值
progress  百分比测量标签，即进度条
~~~

# html标签常用的全局属性，即所有标签都有的属性
~~~
id：      标签的唯一标识，同一html中不能与其他标签的id重复，后续可以通过id对对应的标签设置css样式
class:    标签指定的类名，可以指定多个类名，使用空格分隔不同的class值，后续可以通过对同一类名的标签设置css样式
sytle：   对标签设置css样式
...
- 注意：链接里面不能再放链接，特殊情况链接 a 标签里面可以放块级元素，但是给 a 标签转换一下块级模式最安全

### 行内块元素
在行内元素中有几个特殊的标签：img、input、th、td等，它们同时具有块元素和行内元素的特点。有些资料称它们为行内块元素它们的特点如下

...
# 显示数据包到目标ip或域名的路径
traceroute [ip或域名]
# 显示星号（星号的数量代表了发送的数据包数量），是中间路由的防火墙封掉了ICMP或UDP的返回信息，所以我们得不到什么相关的数据包返回数据
# -n 参数 接ip速度快
# traceroute的原理是第一次发送数据包并且ttl设置为1，这样包到达第一个节点后节点会进行ttl减1，当ttl是0时包会被丢弃并且返回当前丢弃包的节点信息，其中就包含ip信息，然后再进行第二次发送数据包并且设置设置为2，当包丢弃后又获得了第二跳的节点信息，这样依次直到获取链路上所有的ip地址

# 查看arp高速缓存，即ip地址到mac地址的映射表
arp -a
# 会显示主机名称、ip地址、mac地址、网卡名称
